...
- Create a composite index on `category` and `price` to speed up filtering on both columns and reduce the amount of table scanning required.
- Select only the required columns instead of using `SELECT *` to reduce the amount of data the database needs to read and return.
